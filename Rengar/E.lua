...
function E:makeMenu()
    ...
    self.menu:boolean("gapclose", "Use to Gapclose", true)
    self.menu:boolean("afterAA", "Use afterAA", true)
    self.menu:boolean("leap", "Use when leaping", true)
end
...
function E:dmg(tar)
    local res = 0
    if player:spellSlot(self.slot).state ~= SpellState.Ready then
        return res
    end
...
function E:use(pos)
    if pos then
        if player:castSpell(self.type, self.slot, pos) then
            return true
        end
    end
    return false
...
function E:shouldUse(tar)
    ...
    if self.menu.ks:get() and self:canKill(tar) then
        return true
    end
    if self.menu.gapclose:get() and dist <= self.range and dist > player.attackRange + player.boundingRadius and
            player.pos:dist(tar.pathEndPos) > dist then
        return true
    end
    if  Ferocity:getStacks() < 4 then 
        if self.menu.leap:get() and player.isDashing then
            return true
        end
        if self.menu.afterAA:get() and player:isInAutoAttackRange(tar) and orb.core.can_spell_after_attack() then
           return true
        end
    end
    return false
...
function E:onTick()
    ...
    if self:shouldUse(tar) and self:use() then
        return true
    end
